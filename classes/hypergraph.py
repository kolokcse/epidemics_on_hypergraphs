...
class Hypergraph:
    ...
    def __init__(self, nodelist = None, is_nodeattr = False, edgelist = None, is_edgeattr = False):
        self.nodes = []
        self.edges = []
        if nodelist != None:
    ...
    def add_edge(self, raw_edge, is_attr = False, active = False):
        ...
        for node in edge.nodelist:
            if node not in self.nodes:
                node1=self.add_node([[],{'id':node}], True)
                node1.edgelist.append(edge)
            else:    
    ...
    def to_clique_nxGraph_expansion(self):
        n=len(self.nodes)
        m=len(self.edges)
        B=self.incidence_mtx()
        mtx=np.matmul(B,B.transpose())
    ...
    def to_hnx_hypergraph(self, pos=None):
        drawing={}
        for edge in self.edges:
            drawing[edge.attr_list['id']] = [node.attr_list['id'] for node in edge.nodelist]
        H=hnx.Hypergraph(drawing)
        return H
