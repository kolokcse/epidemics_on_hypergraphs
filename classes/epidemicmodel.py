...
class EpidemicModel(object):
    ...
    def __init__(self, nodelist, state_transition_probs, infectious_change_mtx = None,
                    ...
                    vaccination_start_rate=-1, first_vaccination_vector = [], second_vaccination_vector = [],
                    ...
                    model = 'basic', exclude_died = False, stopping_rule=None,
                    node_history_on = False, school_closure=False,
                    state_rates_ages=False, state_rates_degrees=False):        
        self.stopping_rule = stopping_rule
        self.first_vaccination_vector = first_vaccination_vector
        self.second_vaccination_vector = second_vaccination_vector
        self.state_rates_ages=state_rates_ages
        self.state_rates_degrees = state_rates_degrees
        ...
        self.vaccination_start_rate = vaccination_start_rate
    ...
    def get_state_sum_age(self, _state, _age):
        _sum = 0
        for node in self.nodelist():
            if node.attr_list['state'] == _state and node.attr_list['age']==_age:
                _sum += 1
        return _sum
    
    def get_state_rate_age(self, _state, age):
        return self.get_state_sum_age(_state, age)/len(self.nodelist())
    
    def get_state_sum_degree(self, _state, d):
        _sum = 0
        for node in self.nodelist():
            if node.attr_list['state'] == _state and node.degree()==d:
                _sum += 1
        return _sum
        
    def get_state_rate_degree(self, _state,d):
        return self.get_state_sum_degree(_state,d)/len(self.nodelist())
    ...
    def add_node(self, _node):
        ...
        node.attr_list['immunation rate'] = 0
    ...
    def preferential_choice(self, k, i, j,m,t):
        ...
        r = rnd.random()
        index = bisect.bisect(prob_int, r)
        return self.nodelist()[index]
    
    def select_node_girh(self, new_nodelist, hyperedge_data):
        pass
    
    def girh_hyperedge(self, hyperedge_data):
        k = hyperedge_data['edge size']
        new_nodelist=[]
        for i in range(k):
            node=self.select_node_girh(new_nodelist, hyperedge_data)
            new_nodelist.append(node)
    
    def girh_model(self, hyperedge_data, _type):
        m = hyperedge_type['number of edges']
        for i in range(m):
            new_hyperedge=self.girh_hyperedge(hyperedge_data)
    ...
    def preferential_attachment_model_3(self, hyperedge_data, _type, model='BA',edge_prob=1):
        k=0
        while k < 2:
            k = self.sample(hyperedge_data)
        m = hyperedge_data['number of new edges']
    ...
    def girh(self, hyperedge_data, type):
        n=len(self.nodelist())
        coordinates=np.array([np.random.rand(2) for i in range(n)])
        m = hyperedge_data['m']
        alpha = hyperedge_data['alpha']
        if "deg_exp" in hyperedge_data:
            deg_exp = hyperedge_data["deg_exp"]
        else:
            deg_exp = 3.5
        d=powerlaw.Power_Law(xmin=3,parameters=[deg_exp]).generate_random(n)
        w_v=d
        if "size_exp" in hyperedge_data:
            size_exp = hyperedge_data["size_exp"]
        else:
            size_exp = 3.5
        ...
        for i in range(m):
            ...
            for j in range(n):
                if rnd.random()< np.power((w_es[i]*w_v[j]/W),alpha)/np.power(distance.euclidean(c_is[i],coordinates[j]), alpha * 2) :
                    nodelist.append(self.hypergraph.nodes[j])
            self.add_hyperedge_from_nodelist(nodelist, hyperedge_data, type)
            
    def add_hyperedge_from_nodelist(self, nodelist, hyperedge_data, _type):
        _attr_list = {'id' : str(self.edge_id) ,'type' :_type, 'number of infectious' : 0, 'spreading rate' : hyperedge_data['spreading rate']}
        ...
        if  edge.size() < 2:
            ...
            for node in edge.nodelist:
                ...
                if 'immunation rate' in node.attr_list.keys():
                    pi=(1-node.attr_list['immunation rate'])
    ...
    def vaccinate_node(self, node):
        ''' Vaccinates a node with the first dose, sets its vaccination period to 0.'''
        
        node.attr_list['vaccination period'] = 0
    
    def second_vaccinate_node(self, node):
        ''' 
        ...
        returns: None
        '''
        node.attr_list['second vaccination period'] = 0
    ...
    def sort_random(self):
        ''' Shuffles the nodes of the hypergraph nodelist randomly.'''
    ...
    def change_vaccination_time(self,node):
        ...
        if node.attr_list['second vaccination period'] != None:
            node.attr_list['second vaccination period'] += 1
        if node.attr_list['vaccination period'] == 14:
            if self.vaccination_efficient=='total efficient':
                ...
                node.attr_list['immunation rate'] = self.vaccination_efficiency[0]
        if node.attr_list['second vaccination period'] == 14:
            node.attr_list['immunation rate'] = self.vaccination_efficiency[1]
    ...
    def timestep(self, step_index,vacc_start=False):
        ...
        '''
        if len(self.first_vaccination_vector)!=0:
            self.first_vaccination_limit = self.first_vaccination_vector[step_index]
        if len(self.second_vaccination_vector)!=0:
            self.second_vaccination_limit = self.second_vaccination_vector[step_index]
        vacc_ind = 0
        second_vacc_ind = 0
        daily_infections = 0
        ...
        for node in self.nodelist():
            if node.attr_list['state'] != 'd':
                ...
                if self.vaccination_method != None and vacc_start: 
                    ...
                    if vacc_ind < self.first_vaccination_limit +1 :
                        if not node.attr_list['is tested'] and node.attr_list['vaccination period']==None:
                            self.vaccinate_node(node)
                            vacc_ind += 1
                    if second_vacc_ind < self.second_vaccination_limit +1 :
                        if not node.attr_list['is tested'] and node.attr_list['vaccination period']!=None:
                            self.second_vaccinate_node(node)
                            second_vacc_ind += 1
                if self.testing_prob != 0:
                    result = 0
    ...
    def run(self, steps=None):
        ...
        rates['i_s_edge'] =[self.hypergraph.edges[0].attr_list['number of infectious']/self.hypergraph.edges[0].size()]
        for state in self.states:
            rates[state] = [self.get_state_rate(state)]
        if self.state_rates_ages:
            rates_ages = {}
            for state in self.states:
                rates_ages[state] = {}
                for age in range(self.number_of_stratas):
                    rates_ages[state][age]=[self.get_state_rate_age(state,age)]
        ...
        if steps==None:
            stop=False
            while (self.get_infection_rate()>0 or self.get_state_rate('e')>0) and not stop:
                ...
                if self.state_rates_ages:
                    for state in self.states:
                        for age in range(self.number_of_stratas):
                            rates_ages[state][age].append(self.get_state_rate_age(state,age))
                if self.node_history_on:
                    self.node_history.append(self.get_nodes())
                #self.check_infectious_number_validity()
                i += 1
                if self.stopping_rule !=None and i > 100:
                    if rates['infectious'][-1] < self.stopping_rule['infectious'] and rates['d'][-1] > self.stopping_rule['d']:
                        stop=True 
            ...
            for i in range(steps):
                d_i,d_p=self.timestep(i,vacc_on)
                if not vacc_on:
                    if d_p > self.vaccination_start_d_p and i > self.vaccination_start_time and self.vaccination_start_rate < (self.get_state_rate('r')/self.get_state_rate('s')):
                        vacc_on=True
                daily_infections.append(d_i)
                daily_positive_tests.append(d_p)
                rates['infectious'].append(self.get_infection_rate())
                rates['i_s_edge'].append(self.hypergraph.edges[0].attr_list['number of infectious']/self.hypergraph.edges[0].size())
                for state in self.states:
                    rates[state].append(self.get_state_rate(state))
                if self.state_rates_ages:
                    for state in self.states:
        ...
        if self.state_rates_ages:
            return rates_ages, daily_infections, daily_positive_tests, i
        return rates, daily_infections, daily_positive_tests, i
    
    def end_quarantines_vaccination_death(self):
        for node in self.nodelist():
            node.attr_list['vaccination period'] = None
            node.attr_list['second vaccination period'] = None
            if node.attr_list['quarantine time'] > 0:
                node.attr_list['quarantine time'] = 0
                for edge in node.edgelist:
                    if edge.attr_list['type'] != 'f':
