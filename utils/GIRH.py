...
from drive.MyDrive.EpidemicModel.model3.hypergraph import Hypergraph, from_incidence_mtx
...
class GIRH:
    ...
    def draw(self, pos=None):
        ...
        for node_i,node in enumerate(self.hypergraph.nodes):
            pos[node.attr_list['id']]=tuple(self.vertex_locations[node_i])
        for edge_i,edge in enumerate(self.hypergraph.edges):
            pos[edge.attr_list['id']]=tuple(self.edge_locations[edge_i])
        hnx.draw(H, pos=pos)