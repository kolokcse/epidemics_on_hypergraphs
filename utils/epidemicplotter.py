...
def av_cut_10_percent(vec):
        l = len(vec)
        vec.sort()
        return sum(vec[int(l/10):int(l*9/10)])/len(vec[int(l/10):int(l*9/10)])
        
def plotting_av_rates(run_hg, filename=None):
        '''
...
def plotting_av_rates_multicompare(runs, modes, lim=1 ,curve='i',runsg=None ,filename=None):
...
def plot_to_compare_2(rates_hg,rates_g,label_1,label_2,state='infectious',filename=None):
        plt.figure(figsize=(10,7))
        max_rates_hg = []
        min_rates_hg = []
        av_rates_hg = []
...
def plot_1_rates(rates, filename=None):  
        plt.figure(figsize=(10,7))
...
def final_size_distribution_compare(rates_1,rates_2,label_1, label_2, size, _scale='lin'):
        distr_1 = [0]
        for run in rates_1:
...
def plot_distributions(distrs, xlabel, labels, title=None, figsize=(10,6), scale='lin'):
        cols=['r*','bo','g^','kD','mv']
...
def plotting_to_compare_rates(rates, labels, curve, lim, filename=None):
...
def av_numbers_by_keyparameter(runs,labels,title=None, scat=False):
        cols=['r-*','b-o','g-^','k-D','m-v']
